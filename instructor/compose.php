...
<html>
    ...
    <head>
		...
		<script src="../_public/../_public/js/jqBootstrapValidation.js"></script>
		<script>
...
if(isset($_POST['title'], $_POST['recip'], $_POST['message']))
{
	...
	if($_POST['title']!='' and $_POST['recip']!='' and $_POST['message']!='')
	{
		...
		$dn1 = mysql_fetch_array(mysql_query('select count(student_id) as recip, student_id as recipid, (select count(*) from tbl_pm) as npm from tbl_student where student_id="'.$recip.'"'));
		$dn2 = mysql_fetch_array(mysql_query('select count(username) as recip, username as recipid, (select count(*) from tbl_pm) as npm from tbl_parent where username="'.$recip.'"'));
		if($dn1['recip']==1)
		{
...
<?php
		...
		if($dn2['recip']==1)
		{
			...
			if($dn1['recipid']!=$_SESSION['username'])
			{
				...
				$time = time() + (4 * 60 * 60);
				//We send the message
				if(mysql_query('insert into tbl_pm (id, id2, title, user1, user2, message, timestamp, user1read, user2read)values("'.$id.'", "1", "'.$title.'", "'.$_SESSION['username'].'", "'.$dn2['recipid'].'", "'.$message.'", "'.$time.'", "yes", "no")'))
				{
		?>
		<div class="message">The message has successfully been sent.<br />
		<a href="inbox.php">List of my personnal messages</a></div>
