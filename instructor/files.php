<?php
	...
	else if(isset($_REQUEST['myDelete'])) {
		$fileID    = $_REQUEST['myDelete'];
		mysql_query("DELETE FROM tbl_files WHERE file_id='$fileID'")
					   or die(mysql_error()) ;
	}
?>
                            ...
                            <div class="block">
                                <div class="navbar navbar-inner block-header">
                                    ...
                                    <div class="pull-right"><a href="upload.php">
                                      <button class="btn" style="margin-bottom:5px;"> Upload File</button>
                                    </a>
                                    </div>
                                </div>
                                <div class="block-content collapse in">
                                    <table class="table table-striped">
                                        ...
                                        <tbody>
                                            <?php
												$sql2 = mysql_query("SELECT * FROM tbl_files  WHERE class_id='$classID'")
																   or die(mysql_error()) ;
													if(mysql_num_rows($sql2) == 0)
													{?>
														<tr>
															<td><?php echo "No File/s has been uploaded for this class.<br>"; ?></td>
														</tr><?php
													} 
													else
													{
														while($row=mysql_fetch_array($sql2))
														{
															...
															?>
															<td><a href="files.php?myFile=<?php echo $row['file_id']?>"><button class="btn"><i class="icon-download-alt"></i></button></a>
																<a href="files.php?myDelete=<?php echo $row['file_id']?>"><button class="btn"><i class="icon-remove"></i></button></a>
																<a href="files.php?myEdit=<?php echo $row['file_id']?>"><button class="btn"><i class="icon-edit"></i></button></td></a>
															<?php echo '<tr>';
														}
													}
												?>
