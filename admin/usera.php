<?php
	...
	if(!($_SESSION['login'])) {
		...
		$id = $_REQUEST['myVar'];
		$myQuery = mysql_query("SELECT * FROM tbl_student WHERE student_id='$id'");
			...
			if(isset($_POST['actiStud']))
				{
					...
					$acsql =	mysql_query("UPDATE tbl_student SET status='1' WHERE student_id='$id'")
								or die (mysql_error(). "Can't Update :( ");
								$msg = "<script>alert (\"Student's Account is Activated!!\")</script>";
				}
				if(isset($_POST['deacStud']))
				{

					$acsql =	mysql_query("UPDATE tbl_student SET status='0' WHERE student_id='$id'")
								or die (mysql_error(). "Can't Update :( ");
														
					$msg = "<script>alert (\"Student's Account is Deactivated!!\")</script>";
					}

				if(isset($_POST['resetPass'])) {
					$resetPass = "password";
					$resetPassword = md5("password");
					mysql_query("UPDATE tbl_student SET pass='$resetPass', password='$resetPassword' WHERE student_id='$id'");
					$msg = "<script>alert (\"Student's Password has been reset!!\")</script>";
				}
                ...
                <p>                 
                            ...
                            <div class="block">
                                <div class="navbar navbar-inner block-header">
                                    <div class="muted pull-left"><h5>Student</h5></div>
								...
								<form method="post" action="">	
                                 <table>
									<tr>
										<td>Student ID: 
										<td><input type="text" value="<?php echo $student_id ?>" readonly/>
									</tr>
									<tr>
										<td>First Name: 
			...
			$('.notification').click(function() {
				...
				switch($id) {
					case 'notification-sticky':
						$.jGrowl("Student's Account Activated!");
					break;

					case 'notification-header':
						$.jGrowl("Student's Account Deactivated!");
					break;

					default:
						$.jGrowl("Please Update the Student's Account");
					break;
				}
			});
        });
