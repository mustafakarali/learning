...
<html class="no-js">
    ...
    <head>
        <title>Administrator | Achievement Board</title> <link rel="shortcut icon" href="../_public/img/favicon.ico" />
        <!-- Bootstrap -->
                ...
                <p>                 
                            ...
                            <div class="block">
                                <div class="navbar navbar-inner block-header">
                                    <div class="muted pull-left"><h5>Top Student</h5></div>
                                        ...
                                        <tbody>
                                            <?php
													...
													while($row = mysql_fetch_array($sql))
													{
														...
														if(isset($_POST['acti']))
														{
															$activate = mysql_query("INSERT INTO tbl_achievetop(gpa,student_id,first_name,middle_name,last_name,program,status)
																		VALUES ('$a','$b','$c','$d','$e','$f',1)") or die(mysql_error());
															$msg = "<script>alert (\"Rankings are now activated on the homepage!\")</script>";
															/*if($stat != 1)
															{
																$msg = "<script>alert (\"Ranking is Already Activated!\")</script>";
															}*/
														}
														else if(isset($_POST['deac']))
														{
															$deactivate = mysql_query("UPDATE tbl_achievetop SET status='0'") or die(mysql_error());
															$msg = "<script>alert (\"Rankings are now deactivated on the homepage!\")</script>";
															/*if($stat != 0)
															{
																$msg = "<script>alert (\"Ranking is Already Deactivated!\")</script>";
															}*/
														}
													}
												?>
