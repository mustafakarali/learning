...
<div class="span3" id="sidebar">
	...
	<?php
			...
			$gradeID = mysql_result(mysql_query("SELECT status  FROM tbl_notification WHERE category='warning' AND class_class_id='$enrollID' ORDER BY notif_id DESC LIMIT 1"),0);
	...
	<div align="center" style="margin:5px 0 5px 0;">
		...
		?> Your instructor encoded a grade for <?php echo $component.', <font color="red">'.$score.'</font> over '.$items.'.';?>
	    <br>It makes your average in <?php echo $component.' <b>'.$ave.'%</b>.<br>';?> 
		<?php if($ave<70) { if($check) { echo "You need to get atleast <b>$aim%</b> in the next $component.<br>";}
							
					echo "---------------------------------";
				echo "Your final grade for now is ";
				if($grade<70) { echo "<font color=\"red\">$grade</font>.<br>"; }
				else  { echo "$grade.<br>"; }
				
				if(!$check) { echo "You need to get atleast <b>$aim%</b> in the $nameID.<br>"; }} 
		
			mysql_query("UPDATE tbl_notification SET status='0' WHERE class_class_id='$enrollID' AND category='warning'");
		
		
		}
			if($allow){
				if($abs==$allow) {
					echo "ATTENDANCE: You already consumed all the allowable absences. <br> One more absent or late will automatically drop you.
					<br> NOTE: Only $allow allowable absences";
				}
				else if($abs==($allow-1)){
					echo "ATENDANCE: You already have $abs absences.
					<br> NOTE: Only $allow allowable absences";
				}
				else if ($abs>$allow){
					echo "ATENDANCE: You are considered unofficially dropped because of having $abs number of absences. <br>
